...
fun SincMatrix.map(transform: (value: Double) -> Double) =
    this.asRowMajorArray().map {
        transform(it)
...
fun SincMatrix.mapColumns(transform: (colVector: SincMatrix) -> SincMatrix) =
    this.columns.map {
...
fun SincMatrix.mapRows(transform: (rowVector: SincMatrix) -> SincMatrix) =
    this.rows.map {
        val tVal = transform(it)
...
fun SincMatrix.mapRowsToDouble(transform: (rowVector: SincMatrix) -> Double) =
    this.rows.map {
        transform(it)
    }.asSincMatrix(false)

fun SincMatrix.mapColumns(
    resultLen: Int,
    transform: (colVector: SincMatrix) -> SincMatrix,
) = this.columns.map {
    val tVal = transform(it)
...
fun SincMatrix.mapRows(
    resultLen: Int,
    transform: (rowVector: SincMatrix) -> SincMatrix,
) = this.rows.map {
    val tVal = transform(it)
...
fun SincMatrix.mapColumnsToList(transform: (colVector: SincMatrix) -> SincMatrix): List<SincMatrix> =
    this.columns.map {
        val tVal = transform(it)
